...
- **Visualization**: ブラウザ用 Canvas レンダラを `src/python/static/` に同梱。サーバー側でシミュレーションを進め、クライアントはスナップショットを受信して描画するだけです（Sim→View の一方向）。
...
- ブラウザで `http://localhost:8000` を開くと Canvas 上にキューブが表示されます。
