...
`--log-format basic` は従来の最小カラムのみを出力します。`--log-format detailed`（既定）は追加の分析用カラムを含み、`--summary` でパーセンタイルと相関の JSON サマリを書き出せます。`--summary-window` で末尾ウィンドウの tick 数を調整できます。
...
- `avg_energy` / `avg_age`: ????????????
- `groups`: ????????????? 0???????????????
- `neighbor_checks`: ??????????O(N^2) ?????????????
- `tick_ms`: 1 ?????????????????????????????????????`--deterministic-log` ?? 0.000??

`--log-format detailed` ?????????????????:
- `ungrouped`: ??????
- `ungrouped_ratio`: ??????ungrouped / population?
- `births_per_agent` / `deaths_per_agent`: ???????????
- `neighbor_checks_per_agent`: ????? / ???
- `tick_ms_per_agent`: tick_ms / ???
- `avg_speed`: ????
- `avg_stress` / `max_stress`: ?????????????
- `avg_group_size` / `min_group_size` / `max_group_size`: ?????????
- `occupied_cells`: ?????
- `avg_agents_per_cell` / `max_cell_occupancy`: ??????
- `population_density`: population / (world_size^2)
- `group_stride` / `group_stride_threshold`: ?????????????????
- `group_stride_active`: ?????????? 1
- `group_stride_active_agents` / `group_stride_skipped_agents`: ????????????????? / ?????
...
- 環境フィールド: `food_regen_per_second`, `food_from_death`, `pheromone_diffusion_rate` / `pheromone_decay_rate`, `pheromone_deposit_on_birth`, `danger_diffusion_rate` / `danger_decay_rate` など
- フィールド更新頻度: `environment_tick_interval`（既定 6.0 秒）。食料/フェロモン/危険の拡散・減衰をこの周期でバッチ処理し、CPU 負荷を抑えます。
...
`EnvironmentGrid` はセル毎に `food`・`pheromone`（グループ別）・`danger` の 3 フィールドを持ち、`environment_tick_interval` ごとに減衰と隣接セルへの拡散をまとめて行います。
