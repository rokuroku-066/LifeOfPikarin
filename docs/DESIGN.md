...
* **Simulation Core (`src/terrarium/world.py`)**
  ...
  * `EnvironmentGrid` が食料・危険・フェロモン・グループ食料のフィールドを管理し、`environment_tick_interval` 間隔でディフュージョン/減衰をまとめて処理。
...
7. **環境更新**: `environment_tick_interval` ごとにフィールドの拡散・減衰・ノイズ更新をバッチ適用し、不要なグループ層を pruning。
8. **メトリクス記録**: tick 時間、人口、出生/死亡数、平均エネルギー・年齢、グループ数、近傍チェック回数、未所属数を `TickMetrics` として保存し、スナップショットに載せる。

## 4. グループ形成・維持の仕掛け

* **形成**: 未所属で近傍未所属が閾値以上なら確率で新規グループ化し、その場に拠点（base）を記録。閾値は `group_formation_neighbor_threshold`、上限は `max_groups`。
* **採用・離脱**: 近傍多数派へ確率採用、孤立が続くと未所属化 or 新規グループ化。拠点半径内の未所属は弱い引力で帰巣しやすい。
* **分裂**: 近傍仲間が多くストレスが高いと確率で分裂し、新グループへ近傍を引き抜く。ピーク人口到達後は最低グループ数を保証するため、未所属を割り振り seed する。
* **行動バイアス**: 同グループ Cohesion・Alignment は強め、他グループ Separation/回避半径で圧をかけ、未所属は近傍グループとベースに引かれる。拠点は描画とは独立し、決定論を維持。
