...
6. `TickMetrics` を生成して履歴に保存（tick 時間、人口、出生/死亡、平均エネルギー・年齢、グループ数、近傍チェック数、未所属数）。
...
- **色/スケール**: `computeGroupHue` に系譜オフセットを足して色相を決定。エネルギー→明度、速度トレイト→彩度補正。成熟後は加齢で縮小し、繁殖欲求に応じたパルスで明滅＆スケール鼓動を与える。
...
- **長時間確認**: `python -m terrarium.headless --steps 5000 --seed 42 --log artifacts/metrics.csv --log-format detailed --summary artifacts/summary.json` で headless 実行し、ピーク人口・tick 時間・近傍チェックなどが安定していることを確認。
